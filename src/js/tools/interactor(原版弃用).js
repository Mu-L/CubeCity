...
export default class Interactor {
  ...
  _handleBuildMode() {
    ...
    this.focused.setType('ground')
    // 更新周边道路
    this._updateAdjacentRoads(this.focused)
    // 显示成功提示
  ...
  _onKeyDown(event) {
    ...
    if (this.gameState.currentMode === 'relocate' && event.key === 'r') {
      if (this.selected && this.selected.buildingInstance && this.selected.buildingInstance.type !== 'road') {
  ...
  _updateAdjacentRoads(tile) {
    ...
    for (const neighbor of neighbors) {
      if (neighbor && neighbor.buildingInstance && neighbor.buildingInstance.type === 'road') {
        neighbor.buildingInstance.refreshView(city)
      }
    }

    // 如果当前 tile 本身是道路，也需要刷新自己
