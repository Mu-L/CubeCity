<template>
  <div class="flex flex-col w-full h-full">
    <div class="text-center text-base sm:text-lg font-bold text-white mb-2 select-none">M A P</div>
...
function getBuildingIcon(buildingType) {
  ...
  const found = BUILDING_DATA.find(b => b.type === buildingType)
  return found ? found.icon : '❓'
}

// 鼠标悬浮提示
function tileTooltip(x, y, tile) {
  if (tile.type === 'road') return `(${x},${y}) 道路`
  if (tile.building) return `(${x},${y}) 建筑: ${tile.building}`
  if (tile.type === 'grass') return `(${x},${y}) 草地`
  if (tile.type === 'ground') return `(${x},${y}) 地面`
  return `(${x},${y}) 空地`
}
</script>
